%% RIS WITH CRLB PHASE
function  [RMSEposc_CRLB_vect]=CRLB_signalY_IT_startSNR(Hbr, Hrm, iterations,xRIS,yRIS,xM_real,yM_real,Nr, Nt, Nris, phasesRIS_0, SNR,x,M0,d_rm,r_rm,AODrm_grad,AOArm_grad, sigma2, param1, dlX1, net, mean_x, std_x, mean_y, std_y)
% % % % load("Normalize_parameters.mat")
% % % % preloaded_net=importNetworkFromPyTorch('mlp_model.pt');
% % % % param1 = [-1.7217, -1.6806]';
% % % % dlX1 = dlarray(param1,'CB');
% % % % net=initialize(preloaded_net, dlX1);
...
phasesRIS_vect_CRLB=phasesRIS_0; 
...
for it=1:iterations
    ...
    if it==1
        ...
        % Normalization of the positions
        % X_norm=(xM_estimat-mean_x)/var_x; Y_norm=(yM_estimat-mean_y)/var_y;
        % param1 = [X_norm, Y_norm];
        % phasesRIS = predict(net, param1);
       
        X_norm=(xM_estimat-mean_x)/std_x; 
        Y_norm=(yM_estimat-mean_y)/std_y;
        param1 = [X_norm, Y_norm]';
        dlX1 = dlarray(param1,'CB');

        phases= predict(net, dlX1);
        phasesRIS=(extractdata(phases))';
...
if RMSEposc_CRLB_vect(end)>10000
    cond_cumplida=false;
else
    cond_cumplida=true;
end
